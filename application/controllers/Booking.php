...
class Booking extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($dtb < 1) {
            ...
            $data['temp'] = $this->db->query("select image, judul_buku, penulis, penerbit, tahun_terbit,id_buku from te
   mp where id_user='$id_user'")->result_array();
        }
        $data['judul'] = "Data Booking";
        $this->load->view('templates/templates-user-1/header', $data);
        $this->load->view('booking/data-booking', $data);
        $this->load->view('templates/templates-user-1/modal');
        $this->load->view('templates/templates-user-1/footer');
    }
    ...
    public function tambahBooking()
    {
        ...
        if ($databooking > 0) {
            $this->session->set_flashdata('pesan', '<div class="alert alertdanger alertmessage" role="alert">Masih Ada booking buku sebelumnya yang belum diambil.<br> A
bmil Buku yang dibooking atau tunggu 1x24 Jam untuk bisa booking kembali </div>');
    ...
    public function bookingSelesai($where)
    {
        ...
        $this->db->query("UPDATE buku, temp SET buku.dibooking=buku.dibooking+1, buku.stok=buku.sto
k-1 WHERE buku.id=temp.id_buku");
    ...
    public function info()
    {
        ...
        $data['items'] = $this->db->query("select*from booking bo, booking_detail d, buku bu where d.id_booking=bo.i
d_booking and d.id_buku=bu.id and bo.id_user='$where'")->result_array();
        $this->load->view('templates/templates-user-1/header', $data);
    ...
    public function exportToPdf()
    {
        ...
        $data['useraktif'] = $this->ModelUser->cekData(['id' => $this->session->userdata('id_user')])->result();
        $data['items'] = $this->db->query("select*from booking bo, booking_detail d, buku bu where d.id_booking=bo.i
d_booking and d.id_buku=bu.id and bo.id_user='$id_user'")->result_array();
